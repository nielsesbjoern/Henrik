...
const FOOD_STOP_IDS = new Set(foodItemBases.map((f) => f.stopId));
...
export function createStopIcon(
  ...
  return new DivIcon({
    className: "",
    html: `<div class="stop-marker${activeClass}${visitedClass}${foodClass}" style="background-color:${background};position:relative">${cutlery}<span class="stop-marker__label">${markerLabel}</span></div>`,
    iconSize: [44, 44],
    iconAnchor: [22, 22],
    popupAnchor: [0, -28],
  });
